"plot.interarrival" <-
  function(x, width = 11, start=1, rep=0, start.day=1,
    ...
    if(rep>0) {
      ...
      if(rep>=num.fact) { # whole repetitions
        for(n in 1:floor(rep/num.fact))
          seas.boxplot(wet,(num.fact*n+1):(num.fact*(n+1)),"wet",op$col[1])
        n <- n+1
      }
      if(rep%%num.fact!=0) { # non-whole repetitions
        ...
        seas.boxplot(wet,(num.fact*n+1):(num.fact*n+rep%%num.fact),"wet",op$col[1])
      }
    }
    ...
    if(rep>0) {
      ...
      if(rep>=num.fact) { # whole repetitions
        for(n in 1:floor(rep/num.fact))
          seas.boxplot(dry,(num.fact*n+1):(num.fact*(n+1)),"dry",op$col[2])
        n <- n+1
      }
      if(rep%%num.fact!=0) { # non-whole repetitions
        ...
        seas.boxplot(dry,(num.fact*n+1):(num.fact*n+rep%%num.fact),"dry",op$col[2])
      }
    }
